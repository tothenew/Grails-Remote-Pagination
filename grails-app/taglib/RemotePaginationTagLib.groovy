...
static namespace = "gpage"
    ...
    def remotePaginate = {attrs ->
        ...
        if (attrs.update == null)
            throwTagError("Tag [paginate] is missing required attribute [update]")

        if (!attrs.action)
            throwTagError("Tag [sortableColumn] is missing required attribute [action]")
...
// display next link when not on laststep
    ...
    def remoteSortableColumn = {attrs ->
        ...
        if (!attrs.update)
            throwTagError("Tag [sortableColumn] is missing required attribute [update]")
...
// process remaining attributes
        ...
        writer << """>${remoteLink(action: action, update: update, params: linkParams) { title } }
            </th>"""
    }
}
